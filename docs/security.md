...
|---|---|
| `/usr/local/etc/xray/` and its state directories | `root:root`; directories are traversable by the service, but manager state is not service-writable |
| `config.json`, profiles and migration markers | `root:root`, readable by Xray as required |
| Reality and VLESS private key files | `root:root`, mode `0600` |
| public key files and generated metadata | `root:root`, readable where the service or subscription handler needs them |
| `/usr/local/etc/xray/scripts/` and `/usr/local/bin/xrayebator*` | `root:root`, executable only as root-managed scripts |
| `/var/log/xray/` | writable by the Xray runtime account for service logs |

The service account can read the configuration it needs, but cannot replace profiles, markers, root
scripts or private keys. Backups are root-owned and kept separately from the live files. Do not put
unrelated secrets in the Xray state directory.

Runtime configuration changes made by `xrayebator` use a backup, validated atomic write and the
`safe_restart_xray` transaction. The installer and project updater are separate lifecycle programs:
they perform their own validation and controlled restart/rollback steps, so it would be inaccurate to
say that every lifecycle path calls `safe_restart_xray`.
