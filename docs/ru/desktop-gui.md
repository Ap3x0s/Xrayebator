...
SNI и fingerprint подчиняются серверной модели профиля/инбаунда. Поэтому изменение на уровне порта может затронуть другие профили на том же инбаунде; серверная команда сообщает результат и необходимость переподключения.
...
Пароли, пароли sudo, passphrase ключей и содержимое приватных ключей не сохраняются. Они существуют только в активной форме/операции и передаются в main process по необходимости. `electron-store` хранит несекретные метаданные и настройки сервера: локальную карточку, имя пользователя, способ аутентификации, режим привилегий и выбранный путь к ключу. Там же после первого успешного подключения (TOFU) закрепляется SHA-256 fingerprint SSH host key. При последующем несовпадении подключение прекращается до выполнения команд; после осознанной переустановки сервера pin можно явно сбросить в Server settings.
...
SSH connect + проверка host key
        │
        ├─ elevated `id -u` и `/etc/os-release`
        ...
        └─ parse `subscription_url` → fetch subscription → сохранить несекретные метаданные
```
